...
from utils import Pos, parse_int, parse_float, ParseNumberError, trim_comment
from section.enums import HitSoundType, SampleBank, SplineType
from section.hit_objects.slider import PathType, PathControlPoint, SliderPath
...
class SampleBankInfo:
    ...
    def read_custom_sample_bank(self, parts: list[str], banks_only: bool) -> None:
        ...
        try: bank_val = parse_int(parts[0])
        except ParseNumberError: bank_val = 0
        bank = SampleBank(bank_val) if bank_val in (1,2,3) else SampleBank.Normal

        try: add_bank_val = parse_int(parts[1]) if len(parts) > 1 else 0
        except ParseNumberError: add_bank_val = 0
        add_bank = SampleBank(add_bank_val) if add_bank_val in (1,2,3) else SampleBank.Normal

        self.bank_for_normal = bank if bank != SampleBank.None_ else None
        self.bank_for_addition = add_bank if add_bank != SampleBank.None_ else self.bank_for_normal

        if banks_only: return

        if len(parts) > 2 and parts[2]: self.custom_sample_bank = parse_int(parts[2])
        if len(parts) > 3 and parts[3]: self.volume = max(0, parse_int(parts[3]))
        if len(parts) > 4 and parts[4]: self.filename = parts[4]

    def convert_sound_type(self, sound_type: HitSoundType) -> list[HitSampleInfo]:
        samples = []
        if self.filename:
            samples.append(HitSampleInfo(
                None,
                self.filename,
                SampleBank.Normal,
                1,
            ...
            is_layered = (sound_type != HitSoundType.NONE) and not HitSoundType.has_flag(sound_type, HitSoundType.NORMAL)
            samples.append(HitSampleInfo(
...
def convert_points(curve_points: list[PathControlPoint], points: list[str], end_points: str | None, first: bool, offset: Pos):
    path_type = PathType.new_from_str(points[0])

    def read_point(val: str) -> Pos:
        v = val.split(':')
        if len(v) < 2: raise ParseHitObjectsError("invalid point format")
    ...
    while end_idx < len(point_split):
        ...
        if end_idx < len(point_split):
            if point_split[end_idx] and point_split[end_idx][0].isalpha():
                end_point = point_split[end_idx + 1] if end_idx + 1 < len(point_split) else None
                convert_points(curve_points, point_split[start_idx:end_idx], end_point, first, offset)
                start_idx = end_idx
                first = False

    if end_idx > start_idx:
        convert_points(curve_points, point_split[start_idx:end_idx], None, first, offset)
...
class HitObjectsState:
    ...
    def last_object_was_spinner(self) -> bool:
        return self.last_object_type is not None and HitObjectType.has_flag(self.last_object_type, HitObjectType.SPINNER)

    def parse_hit_object(self, line: str) -> None:
        clean_line = trim_comment(line)
        split = clean_line.split(',')
        if len(split) < 5: raise ParseHitObjectsError("invalid line")
        ...
        try:
            ...
            force_new_combo = is_first_obj or self.last_object_was_spinner() or new_combo
            ...
            if HitObjectType.has_flag(type_flag, HitObjectType.CIRCLE):
                ...
                hold = HitObjectHold(
                    pos.x,
                    max(start_time, end_time_raw) - start_time
                )
                kind = hold

            elif HitObjectType.has_flag(type_flag, HitObjectType.SLIDER):
                if len(split) < 7: raise ParseHitObjectsError("Invalid Line for Slider")
            ...
            obj = HitObject(
                start_time,
                kind,
                bank_info.convert_sound_type(sound_type)
            )
