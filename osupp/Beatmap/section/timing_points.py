...
from utils import parse_float, parse_int, ParseNumberError, trim_comment, MAX_PARSE_VALUE
...
class DifficultyPoint:
    ...
    def is_redundant(self, existing: "DifficultyPoint") -> bool:
        rerturn (self.generate_ticks == existing.generate_ticks and
                 abs(self.slider_velocity - existing.slider_velocity) < EPSILON)
...
class SamplePoint:
    ...
    def is_redundant(self, existing: "SamplePoint") -> bool:
        return (self.sample_bank == existing.sample_bank and
...
class EffectPoint:
    ...
    def is_redundant(self, existing: "EffectPoint") -> bool:
        return (self.kiai == existing.kiai and
                abs(self.scroll_speed - existing.scroll_speed) < EPSILON)
...
class ControlPoints:
    ...
    def add_timing(self, point: TimingPoint):
        ...
        if idx < len(self.timing_points) and abs(self.timing_points[idx].time - point.time) < EPSILON:
    ...
    def add_difficulty(self, point: DifficultyPoint):
        if not point.is_redundant(self.difficulty_point_at(point.time)):
            idx = bisect.bisect_left(self.difficulty_points, point.time, key=lambda x: x.time)
            if idx < len(self.difficulty_points) and abs(self.difficulty_points[idx].time - point.time) < EPSILON:
    ...
    def add_effect(self, point: EffectPoint):
        if not point.is_redundant(self.effect_point_at(point.time)):
            idx = bisect.bisect_left(self.effect_points, point.time, key=lambda x: x.time)
            if idx < len(self.effect_points) and abs(self.effect_points[idx].time - point.time) < EPSILON:
    ...
    def add_sample(self, point: SamplePoint):
        if not point.is_redundant(self.sample_point_at(point.time)):
            idx = bisect.bisect_left(self.sample_points, point.time, key=lambda x: x.time)
            if idx < len(self.sample_points) and abs(self.sample_points[idx].time - point.time) < EPSILON:
...
class TimingPointsState:
    __slots__ = ("general_mode", "general_default_sample_bank", "general_default_sample_volume",
                 "pending_time", "pending_timing", "pending_difficulty", "pending_effect", "pending_sample",
                 "control_points")
    ...
    def flush_pending(self):
        if self.pending_timing: self.control_points.add_timing(self.pending_timing)
        if self.pending_difficulty: self.control_points.add_difficulty(self.pending_difficulty)
        if self.pending_effect: self.control_points.add_effect(self.pending_effect)
        if self.pending_sample: self.control_points.add_sample(self.pending_sample)
    ...
    def push_point(self, time: float, point, timing_change: bool):
        ...
        if isinstance(point, TimingPoint):
            if timing_change or self.pending_timing is None: self.pending_timing = point
        elif isinstance(point, DifficultyPoint):
            if not timing_change or self.pending_difficulty is None: self.pending_difficulty = point
        elif isinstance(point, EffectPoint):
            if not timing_change or self.pending_effect is None: self.pending_effect = point
        elif isinstance(point, SamplePoint):
            if not timing_change or self.pending_sample is None: self.pending_sample = point
    ...
    def parse_timing_points(self, line: str) -> None:
        ...
        try:
            ...
            if beat_len < -MAX_PARSE_VALUE: raise ParseNumberError(ParseNumberError.Underflow)
            if beat_len > MAX_PARSE_VALUE: raise ParseNumberError(ParseNumberError.Overflow)
            ...
            sample_volume = parse_int(parts[5]) if len(parts) > 5 else self.general_default_sample_volume
            ...
            if timing_change:
                if math.isnan(beat_len):
                    raise ParseTimingPointsError("beat length cannot be NaN in a timing control point")

                timing = TimingPoint(time, max(6.0, min(60000.0, beat_len)), omit_first_bar, time_signature)
                self.push_point(time, timing, timing_change)

            difficulty = DifficultyPoint(time, max(0.1, min(10.0, speed_multiplier)), not math.isnan(beat_len))
            self.push_point(time, difficulty, timing_change)

            sample = SamplePoint(time, sample_set, max(0, min(100, sample_volume)), custom_sample_bank)
            self.push_point(time, sample, timing_change)

            effect = EffectPoint(time, kiai_mode,
                                 max(0.01, min(10.0, speed_multiplier)) if self.general_mode in (GameMode.Taiko,
                                                                                                 GameMode.Mania) else 1.0)
