# Maintainers

This file lists the current maintainers of `osupp`. Maintainers are responsible for reviewing contributions, triaging issues, and ensuring the overall health and direction of the project.

### Core Maintainers

| Name | GitHub | Role | Areas |
|------|--------|------|-------|
| Error44 | [@Error44](https://github.com/Error44s) | Lead Maintainer | Architecture, PP calculation, mod handling, beatmap parsing, testing, documentation |
| DevilPedrow | [@DevilPedrow](https://github.com/DevilPedrow) | Lead Maintainer | Architecture, PP calculation, mod handling, beatmap parsing, testing, documentation |
| O!Lib Team | [@DevilPedrow](https://github.com/DevilPedrow) [@Error44](https://github.com/Error44s) | Team | General maintenance, code review, community support |

### Responsibilities

Maintainers agree to:

- Review and respond to pull requests within 5 business days
- Triage and respond to new issues within 3 business days
- Help guide the technical direction of the project
- Enforce the [Code of Conduct](CODE_OF_CONDUCT.md)
- Ensure contributions meet the standards outlined in [CONTRIBUTING.md](CONTRIBUTING.md)

### Becoming a Maintainer

Contributors who consistently demonstrate:

- High-quality code contributions
- Thoughtful code reviews
- Active participation in issue discussions

may be invited to become maintainers. Maintainership is granted by consensus of existing core maintainers.

### Emeritus Maintainers

Past maintainers who are no longer active:

*(None yet)*

---

**Contact:** For maintainer-related matters, reach out via GitHub or email at `maintainers@olib.dev`

<p align="center">
	<img src="https://raw.githubusercontent.com/catppuccin/catppuccin/main/assets/footers/gray0_ctp_on_line.svg?sanitize=true" />
</p>

<p align="center">
        <code>&copy 2026-Present <a href="https://github.com/O-Lib">O!Lib Team</a></code>
</p>
